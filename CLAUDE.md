...
安全性のため、一旦すべてのファイルをignoreリストに追加し、必要なものだけを明示的に除外する方式を採用しています。これにより、機密情報や誤ってコミットしたくないファイルがリポジトリに含まれるリスクを減らしています。新しい管理対象ファイルを追加する際は、`.gitignore` の `### not ignore ###` セクションに追加してください。
